...
# st.caption('<a href="mailto:jpattany@gmail.com">For any comments or questions please click here.</a>', unsafe_allow_html=True)
...
tons_prod= st.slider('Minimum number of product traded (tons)', min_value=0, max_value=15000, step=100)
...
if len(year_range) ==0:
    ...
    if year_range=='2015 to 2017':
        ...
        if product=='HS12: 282739 - Cobalt chlorides(2007-2500)':
            ...
            for index, row in df_select.iterrows():
                nt.add_edge(row['country_code_x'], row['country_code_y'], width=5*(row['q_total'])/(df_select['q_total'].sum()))
